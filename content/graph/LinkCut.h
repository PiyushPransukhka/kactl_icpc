...
struct snode {
    ...
    int val, sz, v, sum; // value in node, # nodes in current splay tree
...
//LCT[p]->access();LCT[p]->v += x;LCT[p]->calc(); update : a[p] += x;
//LCT[v]->makeRoot();LCT[u]->access();int ans = LCT[u]->vsub + LCT[u]->v; //subtree sum of u(par v)
//LCT[u]->sumRoot() + LCT[v]->sumRoot() - 2*l->sumRoot() + l->v; // u to v path sum
